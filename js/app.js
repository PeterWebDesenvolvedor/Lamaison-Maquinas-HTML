...
const api = {
  ...
  getFallbackData(endpoint) {
    const fallbackMap = {
      ...
      "/vendas/pendentes": () =>
        JSON.parse(localStorage.getItem("user_sales") || "[]").filter(
          (s) => s.status === "pendente",
        ),
      "/capturas": () =>
        JSON.parse(localStorage.getItem("user_captures") || "[]"),
      "/capturas/pendentes": () =>
        JSON.parse(localStorage.getItem("user_captures") || "[]").filter(
          (c) => c.status === "pendente",
        ),
      "/transacoes": () =>
...
async function loadDashboardStats() {
  ...
  try {
    ...
    const activeNegotiations = transactionsData.filter(
      (t) => t.status === "ativo",
    ).length;
...
async function loadUsuariosTable() {
  ...
  try {
    ...
    if (!users || users.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="empty-state">Nenhum usuário cadastrado</td></tr>';
      return;
    }
...
async function loadProdutosGrid() {
  ...
  try {
    ...
    if (!products || products.length === 0) {
      grid.innerHTML =
        '<div class="empty-state" style="grid-column: 1/-1;"><p>Nenhum produto cadastrado</p></div>';
      return;
    }
...
async function loadFinanceiro() {
  ...
  try {
    ...
    const totalComissoes = transactions.reduce(
      (sum, t) => sum + (t.comissao || 0),
      0,
    );
            ...
            <div class="transactions-section">
                ...
                ${
                  transactions.length === 0
                    ? '<div class="empty-state"><p>Nenhuma transação registrada</p></div>'
                    : transactions
                        .map(
                          (t) => `
...
function renderAprovacoes(container) {
  const pendingSales = JSON.parse(
    localStorage.getItem("user_sales") || "[]",
